We are going to define the way observations accept and reject words. Indeed there is an issue with the notion of representation: an observation is an element of $\,\ualg\,$ and can therefore only interact with \emph{representations} of a word, and there are many possible representation of the same word (in definition \ref{words}, different choices of closed terms lead to different representations).
...
Now, if $\,\varphi\,$ is an automorphism of $\,(\closed)^1\,$, it can be written as $\,\varphi(F\tensor 1)=\psi(F)\tensor 1\,$ with $\,\psi\,$ an automorphism of $\,\closed\,$. We define for $\,F=F_1\tensor\cdots\tensor F_n\tensor 1\in(\closed)^n\,$, $\,\tilde\phi(F):=\psi(F_1)\tensor\cdots\tensor \psi(F_n)\tensor 1\,$ which extends into an automorphism of $\,(\closed)^\infty\,$ by linearity. Finally, we extend $\,\tilde\varphi\,$ to $\,\C A\,$ by $\,\overline\varphi(\sigma F):=\sigma\tilde\varphi(F)\,$
}
