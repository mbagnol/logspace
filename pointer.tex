...
In terms of wirings, the \texttt{MOVE} phase corresponds to the application of the word, whereas the \texttt{SWAP} phase corresponds to the application of the observation.
...
It takes little effort to see that our pointer machines are just a reasonable rearrangement of this model, since it is always possible to encode some information in the states.
...
So if $\,L\in \text{\sc Logspace}\,$, $\,M\,$ is deterministic and can always be chosen to be reversible.
}