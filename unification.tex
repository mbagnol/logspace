...
<<<<<<< HEAD
{\label{unif-logspace}Whether two linear terms $\,t,u\,$ with disjoint sets of variables are unifiable, and if so finding a MGU, can be computed in logarithmic space in the size\footnote{The \emph{size} of a term is the total number of occurrences of symbols in it.} of $\,t, u\,$ on a deterministic Turing machine 
=======
{\label{unif-logspace}
