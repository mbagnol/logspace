...
Therefore one has to ensure that acceptation or rejection is independent of the representation, so that the notion makes the intended sense.

\define[automorphism]
	{An \emph{automorphism} of a $\ast$-algebra $\,\C A\,$ is a linear application $\,\varphi\,:\: \C A\rightarrow\C A\,$ such that for all~$\,F,G\in\C A\,$
	\[\varphi(FG)=\varphi(F)\varphi(G) \qquad \varphi(F^\dagger)=\varphi(F)^\dagger \qquad \text{and }
	\qquad\varphi\,\text{ is injective}\]
